    ...
    public partial class Index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    a.Text = "<a href=\"Index.aspx?Xu=1&bookid=" + dt.Rows[i][0].ToString() + "\" class=\"btn btn-info btn - xs prolong \">续借</a>&nbsp<a  href=\"Index.aspx?Xu=2&bookid=" + dt.Rows[i][0].ToString() + "\" class=\"btn btn-success btn - xs return \">还书</a>";
                    r.Cells.Add(bookid);
                    r.Cells.Add(name);
        ...
        protected void pwdButton_Click(object sender, EventArgs e)
        {
            ...
            if (newPwd.Text != confirmPwd.Text)
            {
                ...
                ConnSql con = new ConnSql();
                string sql = "update bookuser set pwd='" + newPwd.Text.Trim() + "' where id='" + Session["id"].ToString().Trim() + "'";
                int x = con.RunSql(sql);
                if (x == 1)
                {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                ConnSql con = new ConnSql();
                string sql = "update bookuser set status='0' where id='" + Session["id"].ToString().Trim() + "'";
                int x = con.RunSql(sql);
                if (x == 1)
                {
            ...
            {
                ConnSql con = new ConnSql();
                string sql = "update bookuser set status='1' where id='" + Session["id"].ToString().Trim() + "'";
                int x = con.RunSql(sql);
                if (x == 1)
                {
